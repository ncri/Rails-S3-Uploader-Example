h2. Blog Post

"http://www.railstoolkit.com/posts/uploading-files-directly-to-amazon-s3-using-fancyupload":http://www.railstoolkit.com/posts/uploading-files-directly-to-amazon-s3-using-fancyupload

h2. Steps to make the example work:

* Add your s3 bucket and keys to config/amazon_s3.yml
* Enter the correct path in helpers/uploads_helper.rb

pre. var up = new FancyUpload3.Attach('uploader_file_list', '#upload_link', { 
pre.   path: 'http://*yourdomain*/javascripts/fancyupload/source/Swiff.Uploader.swf'@

* Enter the correct path for the progress bar image in javascripts/fancyupload/source/FancyUpload3.Attach.js 

    @var progress = new Element('img', {'class': 'file-progress', src: 'http://*yourdomain*/javascripts/fancyupload/assets/progress-bar/bar.gif'}).inject(this.ui.size, 'after');@

* Make sure a public readable crossdomain.xml exists in the s3 bucket you specified in config/amazon_s3.yml, with the following content (you can also limit access to your domain to be more secure):

pre.<?xml version="1.0"?>

pre.<!DOCTYPE cross-domain-policy SYSTEM

pre."http://www.macromedia.com/xml/dtds/cross-domain-policy.dtd">

pre.<cross-domain-policy>

pre.  <allow-access-from domain="*" secure="false" />

pre.</cross-domain-policy>

* Also make sure a folder named 'test' exists in that bucket. Files will be uploaded to that folder.

